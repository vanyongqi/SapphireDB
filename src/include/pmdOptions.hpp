...
/*
1.日志模块和配置解析模块是一个服务器至关重要的两个模块，
也是最常见的两个模块。diydb的这两个模块实现简单，功能够用，是可以直接借鉴的。

2.很多高级的服务器支持配置文件的动态加载功能，
一般都是通过创建新的工作进程或工作线程来加载新的配置文件，然后让旧的工作进程或工作线程处理完剩余的工作后自动销毁。。

3.另外，db并没有支持日志文件的切割，也就是动态切换日志文件。

4.db的时间戳实现没有注意效率，即没有提供时间戳缓存，导致每次需要读取时间时，都是通过系统调用来实现的。
*/
